...
- **Warehouse release slip**: right after a sale is confirmed — and again anytime from the Anbar tab's "🖨 Sənəd" button — the app opens the release slip on screen, listing the Satış ID, date, customer, salesperson, line items (with the 18% VAT split out) and signature lines, meant to be physically handed to warehouse staff so they release exactly those products. The slip's "🖨 Çap et / PDF saxla" button hands it to the browser's own print dialog, whose "Save as PDF" option works on every desktop and mobile OS.
  - It is deliberately **not** generated as a file in JavaScript. The earlier `jsPDF` + `html2canvas` implementation downloaded a blob, which failed silently in two ways: iOS Safari ignores the `download` attribute on blob URLs, and after a sale the download no longer belonged to the originating click (`confirmSale` awaits session refresh, stock adjustment and Supabase writes first), so browsers dropped it as an unrequested automatic download. Printing needs no user gesture and no CDN library, and the resulting PDF has selectable text instead of a flat rasterised image.
  - Print isolation is handled by the `@media print` block in `index.html`: `body.printing-slip` hides every direct child of `<body>` except `#warehouse-slip-modal`.
